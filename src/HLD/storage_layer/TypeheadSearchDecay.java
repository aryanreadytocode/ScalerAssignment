...
public class TypeheadSearchDecay {
    ...
    public void dayPasses(int decayFactor) {
        Date now = new Date();
        int hrs24 = 24 * 60 * 60 * 1000;
        if (now.getTime() - updateTime.getTime() < hrs24) {
            return;
        }
        updateTime = now;
        updateFrequency(decayFactor, root);
    }

    private void updateFrequency(int decayFactor, TrieNode node) {
        for (int i = 0; i < 26; i++) {
            if (node.childLinks[i] != null) {
                for (Pair pair : node.list){
                    pair.frequency /= decayFactor;
                }
                updateFrequency(decayFactor, node.childLinks[i]);
            }
        }
        return;
    }
    private void updateFrequency1(int decayFactor, TrieNode node) {
        // loop through all the alphabets

        for (int i = 0; i < 26; i++) {
            if (node.childLinks[i] == null)
                continue;
            node = node.childLinks[i];

            // update the map value
            Map<String, Long> map1 = new HashMap<>(node.map);
            for (String key : map1.keySet()) {
                long frequency = node.map.get(key) / decayFactor;
                if (frequency > 0) {
                    node.map.put(key, frequency);
                } else {
                    node.map.remove(key);
                }
            }
            ...
            int j = 0;
            while (j < node.list.size()) {
                long frequency = node.list.get(j).frequency / decayFactor;
                String typeSearch = node.list.get(j).typeSearch;
                if (frequency > 0) {
                    node.list.set(j, new Pair(frequency, typeSearch));
                } else {
                    node.list.remove(j);
                }
                j++;
            }
            updateFrequency(decayFactor, node);
        }
    }

    public static void main(String[] args) {
        TypeheadSearchDecay search = new TypeheadSearchDecay();
        search.incrementSearchTermFrequency("michelleobama", 100);
        search.incrementSearchTermFrequency("michaeljackson", 90);
        search.incrementSearchTermFrequency("michaeljordan", 120);
        search.dayPasses(2);
        search.incrementSearchTermFrequency("mickeymouse", 51);
        System.out.println(Arrays.toString(search.findTopXSuggestion("mic", 2)));
        search.dayPasses(5);
    }
